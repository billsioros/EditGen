...
def view_images_with_texts(
    ...
    images_with_texts = [
        text_under_image(img, txt, font_scale=font_scale)
    ...
    images_with_texts = [image.astype(np.uint8) for image in images_with_texts] + [
        empty_images
    ] * num_empty
    ...
    for i in range(num_rows):
        for j in range(num_cols):
            image_[
                i * (h + offset) : i * (h + offset) + h :,
                j * (w + offset) : j * (w + offset) + w,
            ] = images_with_texts[i * num_cols + j]
...
class AttentionReplace(AttentionControlEdit):
    ...
    def __init__(
        ...
        self.mapper = get_replacement_mapper(prompts, self.tokenizer).to(
...
class AttentionRefine(AttentionControlEdit):
    ...
    def __init__(
        ...
        self.mapper, alphas = (
            self.mapper.to(self.device, self.dtype),
            alphas.to(self.device, self.dtype),
        )
